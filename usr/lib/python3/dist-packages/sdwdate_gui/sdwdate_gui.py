...
class RightClickMenu(QtWidgets.QMenu):
    ...
    def __init__(self, parent=None):
        ...
        self.addSeparator()
        ##главная система
        icon = QtGui.QIcon('/usr/share/icons/sdwdate-gui/Ambox_currentevent.svg.png')
        action = QtWidgets.QAction(icon, "sys-whonix", self)
        action.triggered.connect(show_log)
        self.addAction(action)

        self.addSeparator()
        ##система 2
        icon = QtGui.QIcon('/usr/share/icons/sdwdate-gui/620px-Ambox_outdated.svg.png')
        text = 'anon-whonix'
        ...
        ##система 3
        icon = QtGui.QIcon('/usr/share/icons/sdwdate-gui/Ambox_currentevent.svg.png')
        action = QtWidgets.QAction(icon, "anon-web", self)
        action.triggered.connect(stop_sdwdate)
        self.addAction(action)

        icon = QtGui.QIcon('/usr/share/icons/sdwdate-gui/212px-Timeblock.svg.png')
        action = QtWidgets.QAction(icon, "whonix-workstation", self)
        action.triggered.connect(sys.exit)
        self.addAction(action)
        ##система 4
        icon = QtGui.QIcon('/usr/share/icons/sdwdate-gui/620px-Ambox_outdated.svg.png')
        action = QtWidgets.QAction(icon, "anon-git", self)
        action.triggered.connect(stop_sdwdate)
        self.addAction(action)
        ##система 5
        icon = QtGui.QIcon('/usr/share/icons/sdwdate-gui/620px-Ambox_outdated.svg.png')
        action = QtWidgets.QAction(icon, "anon-pentest", self)
        action.triggered.connect(sys.exit)
        self.addAction(action)
        ...
        action = QtWidgets.QAction(icon, "Restart sdwdate", self)
        action.triggered.connect(restart_sdwdate)
        self.addAction(action)
        ##шутдаун
